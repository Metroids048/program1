...
- 问题库重点是用户保存问题与项目资料
- 简历页右侧必须是正常 AI 对话框
- **Codex Desktop 禁止内置 IAB/Browser/Computer Use**（Windows 会闪退）；Codex 可用外部 Playwright/系统 Edge 脚本 `npm run test:browser-flow` 做用户流验收，仍不得调用内置 Browser 面板（见 ADR-P012）
