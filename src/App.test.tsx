...
describe("App", () => {
  ...
  it("auto-generates a live cue card from final speech text", async () => {
    ...
    await waitFor(() => expect(screen.getByText("检索 JD、简历和问题库")).toBeInTheDocument());
    await waitFor(() => expect(screen.getByRole("button", { name: "记录到面试资料" })).toBeInTheDocument(), { timeout: 2500 });
    expect(screen.getByText("正在结合你的资料生成提词卡。")).toBeInTheDocument();
    expect(fetchMock.mock.calls.filter(([input]) => String(input).includes("/api/copilot/cue-card/stream"))).toHaveLength(1);
  });
  ...
  it("uses the mock interview flow of position list to setup to room and saves a mock report", async () => {
    ...
    expect(screen.getByText("模型面试官思考中...")).toBeInTheDocument();
